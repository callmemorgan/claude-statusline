...
func runConfigure() {
	...
	app.SetBeforeDrawFunc(func(screen tcell.Screen) bool {
		...
		if sw, _ := screen.Size(); sw != lastScreenWidth {
			lastScreenWidth = sw
			flex.ResizeItem(help, footerRows(footerText("list"), sw), 0)
...
// ─── Canvas Configure UI (opt-in via "configure --drawer") ───────────

func runConfigureCanvas() {
	if !term.IsTerminal(int(os.Stdin.Fd())) {
		fmt.Fprintln(os.Stderr, "claude-statusline configure requires an interactive terminal.")
		fmt.Fprintf(os.Stderr, "Edit %s directly, or run from a terminal.\n", configPath())
		os.Exit(1)
	...
	// mutateWithSel and refreshUI are assigned later, after the pane rebuild
	// helpers are set up. The cross-pane mutation closures reference them now
	// and call them once both are initialized.
	var mutateWithSel func(drawerWant, canvasWant string, fn func())
	var refreshUI func(drawerWant, canvasWant string)
	var mutate func(fn func())
	...
	// visibleInDrawer reports whether id appears in the drawer given the
	// current filter query.
	visibleInDrawer := func(id string) bool {
		for _, s := range filterSegments(registeredSegments, filterInput.GetText()) {
			if s.id == id {
				return true
			}
		}
	...
	disableToDrawer := func(id string) {
		drawerWant := ""
		if visibleInDrawer(id) {
			drawerWant = id
		}
		mutateWithSel(drawerWant, "", func() {
	...
	// refreshUI rebuilds both panes and the preview. Non-empty wants move focus
	// to that pane and select the named segment; empty wants preserve the
	// current selection.
	refreshUI = func(drawerWant, canvasWant string) {
		if drawerWant != "" {
			focusPane = "drawer"
		} else if canvasWant != "" {
			focusPane = "canvas"
		}
		if drawerWant == "" && focusPane == "drawer" {
			drawerWant = rememberSel()
		}
		if canvasWant == "" && focusPane == "canvas" {
			canvasWant = rememberSel()
		}
		rebuildDrawer(drawerWant)
		rebuildCanvas(canvasWant)
	...
	updateUI = func() { refreshUI("", "") }

	// mutateWithSel funnels config changes that need to land focus on a
	// specific pane/segment, marking the session dirty and clearing any active
	// preset label.
	mutateWithSel = func(drawerWant, canvasWant string, fn func()) {
		fn()
		dirty = true
		activePreset = ""
		refreshUI(drawerWant, canvasWant)
	}
	mutate = func(fn func()) { mutateWithSel("", "", fn) }
	...
	app.SetBeforeDrawFunc(func(screen tcell.Screen) bool {
		...
		if sw, _ := screen.Size(); sw != lastScreenWidth {
			lastScreenWidth = sw
			flex.ResizeItem(help, footerRows(footerText("canvas"), sw), 0)
			flyoutFlex.ResizeItem(flyoutHelp, footerRows(footerText("flyout"), sw), 0)
		}
		return false
	})
