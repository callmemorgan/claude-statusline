...
- **TUI** (`tui.go`, `tui_list.go`, `tui_direct.go`, `flyout.go`, `tui_pickers.go`, `tui_colorpicker.go`, `tui_text.go`, `tui_help.go`, `keymap.go`, `render_spans.go`) — `claude-statusline configure` opens the default **list-based** editor: segment list (left), description panel, live preview, and floating picker overlays (`tview.Pages`). `claude-statusline configure --direct` (or `v` inside the list UI) switches to the **direct-manipulation** editor, where the Preview itself is the editing surface: a cursor (`curLine`/`curCol`/`cursorID`) highlights one rendered segment on its real line; arrows walk it through segments and across lines, `space` toggles it, `m` enters grab/move mode (arrows relocate the segment in real space — reorder within a line and move to an adjacent line — `enter` drops, `esc` cancels), and `a` opens a palette overlay (the former segment list, filtered to off segments) to insert at the cursor. Selection resolves through `cursorSegment()` over the latest `curSpans` from `buildStatuslineSpans` (a TUI-only, position-aware mirror of `buildStatusline` in `render_spans.go` that returns per-line `segSpan`s; render path untouched). The cursor highlight is painted on top of the REAL render via `paintCursorLine`/`sliceVisible` — never faked. Every config mutation goes through `mutate()` (dirty tracking); footer and help generate from the `keymap` table (footers word-wrap via `footerRows`). tview/tcell/term stay confined to these files.
