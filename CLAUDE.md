...
One Go module, `package main`, split by concern. The binary's subcommands (`cmd.go` dispatch): bare stdin→stdout rendering (how Claude Code invokes it — must never change behavior), `install`/`uninstall`, `configure` (tview TUI), `edit` (layout-DSL buffer editor), `version`, `debug`, `help`.
...
- **Layout DSL editor** (`dsl.go`, `tui_editor.go`) — the `edit` subcommand (`runEditor`) is an additive, fully-separate configuration mode that edits the statusline as a textual layout instead of a segment list. `dsl.go` is pure logic: `parseDSL(text) (config, []dslError)` and `configToDSL(config) string` round-trip through the **same** config model (segments/lines/colors/settings) — one buffer line per render line, `id[key=value, …]` token overrides (`color=` is the reserved primary-color pseudo-key), and `# key: value` directives for top-level scalars. The parser validates ids via `segmentByID`, settings via `settingSpec.coerce`, colors via `validColorSpec`, then callers run `validateConfig` to normalize; round-trip + diagnostic tests live in `dsl_test.go`. `tui_editor.go` is a `tview.TextArea` driving the **real** `buildStatusline` preview on every keystroke (reusing `samplePayload`/`previewState`/`ansiToTview`, with the same `gitStatusPreview`/`gitStashPreview` defer-to-nil dance), plus tab-autocomplete (`dslCompletions`) and inline diagnostics; it saves through `saveConfig`. The empty buffer maps to `[]string{}` (hide all). Editor keys live under the `"editor"` keymap context.
