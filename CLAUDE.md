...
- **TUI** (`tui.go`, `flyout.go`, `tui_pickers.go`, `tui_colorpicker.go`, `tui_text.go`, `tui_help.go`, `tui_autolayout.go`, `keymap.go`) — single segment-list home screen with floating picker overlays (`tview.Pages`); all selection goes through the `visible` slice + `selectedSegment()`; every mutation goes through `mutate()` (dirty tracking); footer and help generate from the `keymap` table (footers word-wrap via `footerRows`). tview/tcell/term stay confined to these files.
- **Auto-layout solver** (`autolayout.go`, overlay in `tui_autolayout.go`) — a DESIGN-TIME packer: the user ranks segments by priority and sets a budget (`autoLayoutBudget`: max width, max lines, density), and `packLayout` greedily packs them onto physical lines using `buildStatusline` as the fit oracle (`segmentRenderWidth` measures each segment color-free at width 0, pinned to line 1), demoting lower-priority segments to later lines and dropping them when `maxLines` is exhausted. Pure/deterministic given `(base, priorities, budget, packMeasureInput)` — unit-tested in `autolayout_test.go`. `applyPackResult` emits a CONCRETE config (`Segments`/`Lines` with every placed segment explicitly pinned/`Reflow = "group"`/`Style` from density) through the existing model; the optional `[auto_layout]` config table persists priorities+budget as design-time metadata only (the render path ignores it). Reached via `L` in the TUI or `configure --auto-layout`.
