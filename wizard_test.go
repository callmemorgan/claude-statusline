...
func TestAssembleWizardConfig_DefaultChoices(t *testing.T) {
	cfg := assembleWizardConfig(config{}, defaultWizardChoices(), testRegistry())
...
func TestAssembleWizardConfig_EmptySelectionHidesEverything(t *testing.T) {
	...
	cfg := assembleWizardConfig(config{}, choices, testRegistry())
...
func TestAssembleWizardConfig_UpdateNoticeAppended(t *testing.T) {
	...
	empty := assembleWizardConfig(config{}, wizardChoices{Categories: map[string]bool{}}, testRegistry())
...
func TestAssembleWizardConfig_UnknownIDsDropped(t *testing.T) {
	...
	cfg := assembleWizardConfig(config{}, choices, trimmed)
...
func TestAssembleWizardConfig_NoDuplicateSegments(t *testing.T) {
	...
	cfg := assembleWizardConfig(config{}, wizardChoices{Categories: all, Density: densityBalanced}, testRegistry())
...
func TestAssembledConfigRendersThroughPipeline(t *testing.T) {
	...
	initSegments(nil)
	cfg := assembleWizardConfig(config{}, defaultWizardChoices(), registeredSegments)
...
func TestDeriveWizardChoices_RoundTripsCustomConfig(t *testing.T) {
	...
	orig := assembleWizardConfig(config{}, wizardChoices{
	...
	reassembled := assembleWizardConfig(orig, derived, registeredSegments)
...
func TestAssembleWizardConfig_PreservesUnrelatedFields(t *testing.T) {
	padding := 2
	duration := 30
	announce := true
	base := config{
		Reflow:     "group",
		ColorDepth: "256",
		Style:      styleConfig{Separator: "powerline", SeparatorCustom: ">", Padding: &padding},
		Update:     updateConfig{Mode: "off"},
		ReleaseNotes: releaseNotesConfig{
			Announce:        &announce,
			DurationSeconds: &duration,
		},
		ThemeColors: map[string]string{"model": "#ff0000"},
		Colors:      map[string]string{"directory": "#00ff00"},
		Settings: map[string]map[string]any{
			"git-branch": {"git_status_ttl_sec": 60},
		},
		Plugins: []pluginDef{
			{ID: "myplugin", Command: "echo test"},
		},
		Segments: []string{"directory", "model", "myplugin"},
	}

	choices := defaultWizardChoices()
	choices.GitStatus = true
	cfg := assembleWizardConfig(base, choices, testRegistry())

	if cfg.Reflow != "group" {
		t.Errorf("reflow not preserved: got %q", cfg.Reflow)
	}
	if cfg.ColorDepth != "256" {
		t.Errorf("color_depth not preserved: got %q", cfg.ColorDepth)
	}
	if cfg.Style.Separator != "powerline" || cfg.Style.SeparatorCustom != ">" || cfg.Style.Padding == nil || *cfg.Style.Padding != 2 {
		t.Errorf("style not preserved: got %+v", cfg.Style)
	}
	if cfg.Update.Mode != "off" {
		t.Errorf("update.mode not preserved: got %q", cfg.Update.Mode)
	}
	if cfg.ReleaseNotes.Announce == nil || !*cfg.ReleaseNotes.Announce || cfg.ReleaseNotes.DurationSeconds == nil || *cfg.ReleaseNotes.DurationSeconds != 30 {
		t.Errorf("release_notes not preserved: got %+v", cfg.ReleaseNotes)
	}
	if cfg.ThemeColors["model"] != "#ff0000" {
		t.Errorf("theme_colors not preserved: got %v", cfg.ThemeColors)
	}
	if cfg.Colors["directory"] != "#00ff00" {
		t.Errorf("colors not preserved: got %v", cfg.Colors)
	}
	if got := cfg.Settings["git-branch"]["git_status_ttl_sec"]; got != 60 {
		t.Errorf("unrelated git-branch setting not preserved: got %v", got)
	}
	if len(cfg.Plugins) != 1 || cfg.Plugins[0].ID != "myplugin" {
		t.Errorf("plugins not preserved: got %+v", cfg.Plugins)
	}
	if !contains(cfg.Segments, "myplugin") {
		t.Errorf("custom/plugin segment ID not preserved: got %v", cfg.Segments)
	}

	// The base config itself must not be mutated.
	if base.Settings["git-branch"]["git_status"] != nil {
		t.Errorf("base config was mutated: git_status added")
	}
	if len(base.Segments) != 3 {
		t.Errorf("base Segments slice was mutated: got %v", base.Segments)
	}
}

func TestAssembleWizardConfig_HonorsSegmentSemantics(t *testing.T) {
	plugin := pluginDef{
		ID:      "p",
		Command: "echo p",
		Fields:  []pluginField{{ID: "p.field"}},
	}

	// nil Segments means "defaults": the wizard builds its category layout and
	// auto-appends plugin segment IDs just like mergeWithDefaults.
	nilBase := config{Segments: nil, Plugins: []pluginDef{plugin}}
	nilCfg := assembleWizardConfig(nilBase, defaultWizardChoices(), testRegistry())
	if nilCfg.Segments == nil {
		t.Fatal("nil Segments base should produce an explicit segment list")
	}
	if !contains(nilCfg.Segments, "p.field") {
		t.Errorf("plugin field segment ID not auto-appended for nil Segments: got %v", nilCfg.Segments)
	}
	// When a plugin defines fields, only the field IDs are emitted (the bare
	// plugin ID is not a segment), matching mergeWithDefaults behavior.
	if contains(nilCfg.Segments, "p") {
		t.Errorf("bare plugin ID should not be a segment when fields exist: got %v", nilCfg.Segments)
	}

	// Empty Segments means "hide everything": no segments, but plugins survive.
	emptyBase := config{Segments: []string{}, Plugins: []pluginDef{plugin}}
	emptyCfg := assembleWizardConfig(emptyBase, defaultWizardChoices(), testRegistry())
	if emptyCfg.Segments == nil {
		t.Fatal("empty Segments base must stay non-nil empty")
	}
	if len(emptyCfg.Segments) != 0 {
		t.Errorf("empty Segments base should yield no segments; got %v", emptyCfg.Segments)
	}
	if len(emptyCfg.Plugins) != 1 {
		t.Errorf("plugins should survive even with empty Segments: got %+v", emptyCfg.Plugins)
	}

	// Non-empty Segments preserve plugin/custom segments and remove unselected
	// category segments while adding selected ones.
	customBase := config{Segments: []string{"myplugin", "directory"}}
	customCfg := assembleWizardConfig(customBase, wizardChoices{
		Categories: enabledSet("model"),
		Density:    densityBalanced,
	}, testRegistry())
	if !contains(customCfg.Segments, "myplugin") {
		t.Errorf("custom segment not preserved: got %v", customCfg.Segments)
	}
	if contains(customCfg.Segments, "directory") {
		t.Errorf("unselected category segment should be removed: got %v", customCfg.Segments)
	}
	if !contains(customCfg.Segments, "model") {
		t.Errorf("selected category segment should be added: got %v", customCfg.Segments)
	}
}

func TestAssembleWizardConfig_GitStatusToggleRespectsChoice(t *testing.T) {
	base := config{
		Segments: []string{"git-branch"},
		Settings: map[string]map[string]any{
			"git-branch": {"git_status": true, "git_status_ttl_sec": 60},
		},
	}

	// Turning git status off should remove the git_status key while preserving
	// unrelated git-branch settings.
	off := assembleWizardConfig(base, wizardChoices{
		Categories: enabledSet("git"),
		Density:    densityBalanced,
		GitStatus:  false,
	}, testRegistry())
	if s, ok := off.Settings["git-branch"]; ok {
		if _, has := s["git_status"]; has {
			t.Errorf("git_status should be removed when GitStatus=false; got %v", s)
		}
		if s["git_status_ttl_sec"] != 60 {
			t.Errorf("unrelated git-branch setting not preserved: got %v", s)
		}
	}

	// Turning it on should set git_status true and still preserve the TTL.
	on := assembleWizardConfig(base, wizardChoices{
		Categories: enabledSet("git"),
		Density:    densityBalanced,
		GitStatus:  true,
	}, testRegistry())
	if on.Settings["git-branch"]["git_status"] != true {
		t.Errorf("git_status should be true when GitStatus=true; got %v", on.Settings["git-branch"]["git_status"])
	}
	if on.Settings["git-branch"]["git_status_ttl_sec"] != 60 {
		t.Errorf("unrelated git-branch setting not preserved: got %v", on.Settings["git-branch"]["git_status_ttl_sec"])
	}
}
